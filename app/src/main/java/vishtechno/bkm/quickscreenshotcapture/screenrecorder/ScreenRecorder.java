...
package vishtechno.bkm.quickscreenshotcapture.screenrecorder;
...
import static vishtechno.bkm.quickscreenshotcapture.services.ScreenRecord_Icon.tryPause;
...
public class ScreenRecorder {
    ...
    private void writeSampleData(int track, MediaCodec.BufferInfo buffer, ByteBuffer encodedData) {
        ...
        if (encodedData != null) {
            ...
            if (!tryPause) {
                buffer.presentationTimeUs = ScreenRecord_Icon.getPTSUs();
                try{ mMuxer.writeSampleData(track, encodedData, buffer); }catch (Exception e){ showLog(e.toString()); }
                ScreenRecord_Icon.prevOutputPTSUs = buffer.presentationTimeUs;
            }
            if (VERBOSE)
